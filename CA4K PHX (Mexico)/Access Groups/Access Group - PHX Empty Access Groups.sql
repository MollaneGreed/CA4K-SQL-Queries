...
SELECT DISTINCT
  ...
  FROM [CardAccessLiveConfigurationsMX].[dbo].[ca_vw_AccessGroupsGetForParam] AS mag
  WHERE mag.[AccessGroupNo] NOT IN
    (SELECT DISTINCT
       b.AGroupNo
		FROM [CardAccessLiveConfigurationsMX].[dbo].[ca_vw_BadgeAccessList] AS b
    )
ORDER BY mag.[AccessGroupNo] ASC