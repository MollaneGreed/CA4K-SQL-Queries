...
SELECT 
	...
	COALESCE(
		(
			...
			FROM [CardAccessArchiveEventsMX].dbo.[DBAudit] [dba]
			LEFT JOIN [CardAccessLiveConfigurationsMX].[dbo].[Operators] [dbaopr] ON [dbaopr].[OperatorID] = [dba].[LastOperator]
			CROSS APPLY
				(SELECT 
					TRY_CAST(
...
FROM 
	...
	INNER JOIN 
		[CardAccessLiveConfigurationsMX].[dbo].[Operators] [opr] ON [opr].[OperatorID] = [bdg].[LastOperator]
	WHERE[per].[SSN] IN 
		(
		SELECT
			[per].[SSN]
...
SELECT 
	...
	FROM [stg].[caPerson] [per]
		...
		INNER JOIN [CardAccessLiveConfigurationsMX].[dbo].[Operators] [opr] ON [opr].[OperatorID] = [per].[LastOperator]
...
SELECT 
    ...
    (
        SELECT TOP 1 
            [dbaopr].[OperLoginName]
        FROM [CardAccessArchiveEventsMX_2].dbo.[DBAudit] [dba]
        LEFT JOIN [CardAccessLiveConfigurationsMX].[dbo].[Operators] [dbaopr] ON [dbaopr].[OperatorID] = [dba].[LastOperator]
        CROSS APPLY
            (SELECT 
                TRY_CAST(
...
SELECT 
	...
	FROM [stg].[caPerson] [per]
		...
		INNER JOIN [stg].[caBadge] [bdg] ON [bdg].[PersonID] = [per].[PersonID]
		LEFT OUTER JOIN [CardAccessLiveConfigurationsMX].[dbo].[BadgeAccess] [bdgacc] ON [bdgacc].[Badge] = [bdg].[Badge]
...
SELECT 
	...
	FROM [stg].[caPerson] [per]
		INNER JOIN [dbo].[dimEmployeeAllFutureHireTermV2] [emp] ON [emp].[EmployeeID] = TRY_CONVERT(INT, [per].[SSN])
			AND [emp].[TerminationDate] IS NULL
		INNER JOIN [stg].[caBadge] [bdg] ON [bdg].[PersonID] = [per].[PersonID]
		INNER JOIN [CardAccessLiveConfigurationsMX].[dbo].[BadgeAccess] [bdgacc] ON [bdgacc].[Badge] = [bdg].[Badge]
			AND [bdgacc].[Facility] = [bdg].[Facility]
		INNER JOIN [CardAccessLiveConfigurationsMX].[dbo].[MAccGrp] [magrp] ON [magrp].[AgrpNo] = [bdgacc].[AGroupNo]
			AND [magrp].[Description] NOT LIKE 'AG-CA%-NoAccess'
		LEFT OUTER JOIN [CardAccessLiveConfigurationsMX].[dbo].[AccGrp] [agrp] ON [agrp].[Agno] = [magrp].[AgrpNo]
