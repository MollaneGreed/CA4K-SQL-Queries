...
--Count of US ArchiveEvents_2 per Reader
    ...
    FROM [CardAccessArchiveEventsMX_2].[dbo].[Event] e
...
--Top 10 US Archive_2 events with all columns visible
    ...
    Select Top (10)
    *
    FROM [CardAccessArchiveEventsMX].[dbo].[Event] e
...
--All US readers ordered by panel
    ...
    FROM [CardAccessLiveConfigurationMX].[dbo].Reader r
    ORDER BY r.PnlRef;
...
--Top 10 US ArchiveEvents_2 Events with Reader Names from ArchiveConfig Joined with e.DeviceNo = r.RdrNo
    ...
    COUNT(*) AS 'Event Count'
    FROM [CardAccessArchiveEventsMX_2].[dbo].[Event] e
    INNER JOIN [CardAccessArchiveConfigurationMX].[dbo].[Reader] r ON (e.DeviceNo = r.RdrNo AND e.PnlNo = r.PnlRef)
...
--Top 10 US ArchiveEvents_2 Events with Reader Names from ArchiveConfig Joined with r.RdrNo = e.DeviceNo
    ...
    FROM [CardAccessArchiveEventsMX].[dbo].[Event] e
    JOIN [CardAccessArchiveConfigurationMX].[dbo].[Reader] r ON r.RdrNo = e.DeviceNo
    --WHERE e.edate >= '2023-10-01' AND e.edate < '2023-11-01'
    GROUP BY e.PnlNo, e.DeviceNo, r.Description;


--Check for Events between 2 datas
    SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
    SELECT
    FORMAT(e.EDate,'yyyy-MM-dd') AS 'Event Date',
    COUNT(*) AS 'Event Count'
    FROM [CardAccessArchiveEventsMX_2].[dbo].[Event] e
    WHERE e.EDate BETWEEN '2023-10-01' AND  '2023-10-23'
    GROUP BY FORMAT(e.EDate,'yyyy-MM-dd') 'Event Date' AND 'Event Count';
