-- Date Filters
  -- Set the target date
    DECLARE @TargetDate DATE = '2025-06-22';
...
-- Filter Details
  -- Set Description Filter
    DECLARE @Description VARCHAR(100) = '3000568';
  -- Set Operator Filter
...
-- Archive Config
  ...
  SELECT
    ...
    [OldData],
    [NewData]
  FROM [CardAccessarchiveConfigurationsMX].[dbo].[DBAudit]
    WHERE [Actions] IN ('U','D','I')
    AND [RevisionStamp] BETWEEN @Start AND @End
    AND (@DescriptionFilter <> 'True' OR [Description] LIKE '%' + @Description + '%')
    AND (@OperatorFilter <> 'True' OR [OperatorName] LIKE '%' + @Operator + '%')
...
FROM CombinedData AS cd
 LEFT JOIN [CardAccessArchiveConfigurationsMX].[dbo].[Operators] AS o on cd.[OperatorName] = o.[OperLoginName]
