...
SELECT DISTINCT
    ...
    LEFT JOIN [CardAccessLiveConfigurationsMX].[dbo].[Operators] AS o ON o.[OperatorID] = r.[LastOperator]
    LEFT JOIN [CardAccessLiveConfigurationsMX].[dbo].[DevicePartitions] AS dp ON dp.[caObjectID] = r.[caObjectID]
    LEFT JOIN [CardAccessLiveConfigurationsMX].[dbo].[APBAreas] AS apb_in ON apb_in.[AreaNo] = r.[APBEntry]
    LEFT JOIN [CardAccessLiveConfigurationsMX].[dbo].[APBAreas] AS apb_out ON apb_out.[AreaNo] = r.[APBExit]
WHERE r.[PnlRef] > 0
    --AND r.[NoOTL] = 0
