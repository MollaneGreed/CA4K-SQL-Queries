-- Run from the PHX SQL Server.
/*
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
SELECT
    t1.Description AS 'Reader Name',
    t1.PnlNo AS 'Panel Number',
    t1.DeviceNo AS 'Reader Number',
    COUNT(t1.Description) AS 'Event Count'
FROM CardAccessLiveEventsMX.dbo.Event t1
LEFT JOIN CardAccessArchiveEventsMX.dbo.Event t2
...
WHERE t1.Class = 'DOOR Open too Long' -- Filter by the Door Open too Long event
  AND t1.EDate >= DATEADD(DAY, -1, GETDATE()) -- Filter for Live table last 24 hours
  AND t1.EDate <= GETDATE()
GROUP BY t1.Description, t1.PnlNo, t1.DeviceNo
...
LEFT JOIN CardAccessArchiveEventsMX.dbo.Event t2
  ...
  AND t2.EDate >= DATEADD(DAY, -1, GETDATE()) -- Filter Archive table for last 24 hours
  AND t2.EDate <= GETDATE()
LEFT JOIN CardAccessLiveConfigurationsMX.dbo.Panel t3 
